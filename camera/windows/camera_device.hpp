...
HRESULT CreateCaptureEngine(IMFCaptureEngine** engine)
{
    ...
    return capture_engine_class_factory->CreateInstance(CLSID_MFCaptureEngine, IID_PPV_ARGS(engine));
}
    ...
    MediaFormatConfiguration* media_format_configuration)
{
    ...
    for (const auto& kMediaFormatConfiguration : kMediaFormatConfigurationMap) {
        if (kMediaFormatConfiguration.mf_source_media_subtype == mf_source_media_subtype) {
...
HRESULT GetMFSinkMediaSubtype(IMFMediaType* source_media_type, GUID* mf_sink_media_subtype, bool* passthrough)
{
    GUID source_subtype;
...
HRESULT ConvertToVideoSinkMediaType(IMFMediaType* source_media_type, IMFMediaType* sink_media_type, UINT32 frame_rate)
{
    ...
    GUID mf_sink_media_subtype;
    hr = GetMFSinkMediaSubtype(source_media_type, &mf_sink_media_subtype, &passthrough);
    if (FAILED(hr))
        return hr;
...
struct CapabilityWin
{
    ...
    CapabilityWin(int media_type_index, const VideoCaptureFormat& format, const BITMAPINFOHEADER& info_header)
        : media_type_index(media_type_index)
        , supported_format(format)
    ...
    CapabilityWin(int media_type_index, const VideoCaptureFormat& format, int stream_index)
        : media_type_index(media_type_index)
        , supported_format(format)
        , info_header()
...
bool CompareCapability(const VideoCaptureFormat& requested, const VideoCaptureFormat& lhs, const VideoCaptureFormat& rhs)
{
    ...
    const CapabilityList& capabilities)
{
    ...
    for (const CapabilityWin& capability : capabilities) {
        if (CompareCapability(requested, capability.supported_format, best_match->supported_format)) {
...
public:
    ...
    {
        BOOL is_contiguous;
        return pitch_ > 0 && SUCCEEDED(buffer_2d_->IsContiguousFormat(&is_contiguous)) && is_contiguous && (length_ || SUCCEEDED(buffer_2d_->GetContiguousLength(&length_)));
    }
    ...
    bool Lock2DSize()
    {
        ...
        if (FAILED(buffer_.As(&buffer_2d_2)))
            return false;
        BYTE* data_start;
        return SUCCEEDED(buffer_2d_2->Lock2DSize(MF2DBuffer_LockFlags_Read, &data_, &pitch_, &data_start, &length_));
    }
...
public:
    ...
    bool GetFrameRateFromMediaType(IMFMediaType* type, float* frame_rate)
    {
        ...
        if (FAILED(MFGetAttributeRatio(type, MF_MT_FRAME_RATE, &numerator, &denominator)) || !denominator) {
            return false;
        }
    ...
    bool GetFormatFromSourceMediaType(IMFMediaType* source_media_type, bool photo, VideoCaptureFormat* format)
    {
        ...
        if (FAILED(source_media_type->GetGUID(MF_MT_MAJOR_TYPE, &major_type_guid)) || (major_type_guid != MFMediaType_Image && (photo || !GetFrameRateFromMediaType(source_media_type, &format->frame_rate)))) {
            return false;
        }

        GUID sub_type_guid;
        if (FAILED(source_media_type->GetGUID(MF_MT_SUBTYPE, &sub_type_guid)) || !GetFrameSizeFromMediaType(source_media_type, &format->frame_size) || !GetPixelFormatFromMFSourceMediaSubtype(sub_type_guid, &format->pixel_format)) {
            return false;
        }
    ...
    HRESULT FillCapabilities(IMFCaptureSource* source, CapabilityList* capabilities)
    {
        ...
        for (DWORD stream_index = 0; stream_index < stream_count; stream_index++) {
            ...
            if (stream_category != MF_CAPTURE_ENGINE_STREAM_CATEGORY_VIDEO_PREVIEW && stream_category != MF_CAPTURE_ENGINE_STREAM_CATEGORY_VIDEO_CAPTURE) {
                continue;
            }
    ...
    void AllocateAndStart(int width, int height, UINT32 frame_rate, VideoPixelFormat& pixel_format)
    {
        ...
        if (best_match_video_capability.supported_format.frame_size.width() != width || best_match_video_capability.supported_format.frame_size.height() != height) {
            assert(0);
        }
        ...
        hr = source->GetAvailableDeviceMediaType(best_match_video_capability.stream_index, best_match_video_capability.media_type_index, &source_video_media_type);
        if (FAILED(hr)) {
            assert(0);
        }
        ...
        hr = preview_sink->AddStream(best_match_video_capability.stream_index, sink_video_media_type.Get(), nullptr, &dw_sink_stream_index);
        if (FAILED(hr)) {
            assert(0);
        }

        hr = preview_sink->SetSampleCallback(dw_sink_stream_index, video_callback_.Get());
        if (FAILED(hr)) {
            assert(0);
        }
