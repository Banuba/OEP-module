...
void yuv_converter::convert(uint32_t gl_texture, int width, int height, yuv_converter::yuv_data& output)
{
    ...
    if (output.data.get() == nullptr || output.size < stride * m_height * 3 / 2) {
        output.size = stride * m_height * 3 / 2;
    ...
    m_shader.set_uniform("plane_coef", m_y_plane_coefs[0], m_y_plane_coefs[1],
            m_y_plane_coefs[2], m_y_plane_coefs[3]);
    ...
    m_shader.set_uniform("plane_coef", m_u_plane_coefs[0], m_u_plane_coefs[1],
            m_u_plane_coefs[2], m_u_plane_coefs[3]);
    glViewport(0, m_height, half_viewport_width, half_height);
    glDrawArrays(GL_TRIANGLE_STRIP, m_draw_indent, drawing_plane_vert_count);
    m_shader.set_uniform("plane_coef", m_v_plane_coefs[0], m_v_plane_coefs[1],
            m_v_plane_coefs[2], m_v_plane_coefs[3]);
